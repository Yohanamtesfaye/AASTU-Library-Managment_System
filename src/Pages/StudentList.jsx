...
const StudentsList = () => {
  ...
  const generateReport = () => {
    ...
    students.forEach((student) => {
      ...
      const exitDate = student.exitTime ? new Date(student.exitTime).toDateString() : entryDate
      ...
      const exitHour = student.exitTime ? new Date(student.exitTime).getHours() : 23
    ...
    const averageDuration =
      visitDurations.length > 0 ? visitDurations.reduce((a, b) => a + b, 0) / visitDurations.length : 0
    const longestVisit =
      visitDurations.length > 0
        ? {
            duration: Math.max(...visitDurations),
            student: students.find(
              (s) => (new Date(s.exitTime) - new Date(s.entryTime)) / (1000 * 60 * 60) === Math.max(...visitDurations),
            ),
          }
    ...
    const shortestVisit =
      visitDurations.length > 0
        ? {
            ...
            student: students.find(
              (s) => (new Date(s.exitTime) - new Date(s.entryTime)) / (1000 * 60 * 60) === Math.min(...visitDurations),
            ),
          }
        : { duration: 0, student: null }
    const studentVisits = {}
    ...
    const mostFrequentVisitor = Object.entries(studentVisits).sort(([, a], [, b]) => b - a)[0]
    const mostFrequentVisitorStudent = students.find((student) => student.id === mostFrequentVisitor[0])
    const totalUniqueVisitors = new Set(students.map((student) => student.id)).size
    ...
    return {
      ...
      peakHour,
      booksBorrowed: Object.entries(booksBorrowedByDay).map(([date, count]) => ({ date, count })),
      averageDuration,
      longestVisit,
      shortestVisit,
      mostFrequentVisitor: { student: mostFrequentVisitorStudent, visits: Number.parseInt(mostFrequentVisitor[1]) },
      totalUniqueVisitors,
    }
  }
  ...
  const lastCheckedInStudent = students.find((student) => student.exitTime === null)
  ...
  return (
    <div className="p-6 h-[100vh] overflow-y-auto scrollbar-hide bg-gray-100">
      <div className="mb-6">
        ...
        <p className="text-gray-600 mt-2">Track student entries and exits from the library</p>
      </div>

      {/* Last Checked In Student Card */}
      {lastCheckedInStudent && (
        <div className="mb-6 bg-white rounded-lg shadow-md p-6">
          <h2 className="text-2xl font-semibold text-[#1a365d] mb-4">Last Checked In Student</h2>
          <div className="flex items-center">
            ...
            />
            <div>
              <h3 className="text-xl font-semibold text-[#1a365d]">{lastCheckedInStudent.name}</h3>
        ...
        {currentStudents.map((student) => (
          ...
          >
            <div className="flex items-center p-4 border-b">
              ...
              />
              <div>
                <h3 className="text-lg font-semibold text-[#1a365d]">{student.name}</h3>
                <p className="text-sm text-gray-600">{student.id}</p>
              </div>
              <div className={`ml-auto px-2 py-1 rounded-full text-xs font-semibold ${getStatusColor(student.status)}`}>
                {student.status}
              </div>
            </div>
            <div className="p-4">
              <div className="space-y-2 text-sm">
                ...
                <p className="flex items-center">
                  ...
                  <span className="ml-2">{calculateDuration(student.entryTime, student.exitTime)}</span>
                </p>
                <p className="flex items-center">
                  <Clock className="h-4 w-4 mr-2 text-gray-500" />
        ...
        {Array.from({ length: Math.ceil(filteredStudents.length / studentsPerPage) }, (_, i) => (
          <button
            ...
            className={`px-3 py-1 rounded ${
              currentPage === i + 1 ? "bg-[#1a365d] text-white" : "bg-gray-200 text-gray-700 hover:bg-gray-300"
      ...
      {showReport && (
        <div className="mt-8 bg-white p-6 rounded-lg shadow-md">
          <h2 className="text-2xl font-semibold mb-4 text-[#1a365d]">Usage Analysis Report</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            <div>
              <h3 className="text-lg font-semibold mb-2 text-[#1a365d]">Unique Users per Day</h3>
              <ul className="space-y-2">
                {report.uniqueUsers.map(({ date, count }) => (
                  <li key={date} className="flex justify-between items-center bg-gray-50 p-2 rounded">
                    <span className="text-gray-600">{date}: </span>
                    <span className="font-semibold text-[#1a365d]">{count}</span>
                  </li>
                ))}
              </ul>
            </div>
            <div>
              <h3 className="text-lg font-semibold mb-2 text-[#1a365d]">Peak Usage Hour</h3>
              <p className="text-4xl font-bold text-[#1a365d]">{report.peakHour}:00</p>
            </div>
            <div>
              <h3 className="text-lg font-semibold mb-2 text-[#1a365d]">Books Borrowed per Day</h3>
              <ul className="space-y-2">
            ...
            <div>
              <h3 className="text-lg font-semibold mb-2 text-[#1a365d]">Average Visit Duration</h3>
              <p className="text-4xl font-bold text-[#1a365d]">{report.averageDuration.toFixed(2)} hours</p>
            </div>
            <div>
              <h3 className="text-lg font-semibold mb-2 text-[#1a365d]">Longest Visit</h3>
              <p className="text-sm">
                <span className="font-semibold">{report.longestVisit.student.name}</span>
                <br />
                Duration: {report.longestVisit.duration.toFixed(2)} hours
              </p>
            </div>
            <div>
              <h3 className="text-lg font-semibold mb-2 text-[#1a365d]">Shortest Visit</h3>
              <p className="text-sm">
                <span className="font-semibold">{report.shortestVisit.student.name}</span>
                <br />
                Duration: {report.shortestVisit.duration.toFixed(2)} hours
              </p>
            </div>
            <div>
              <h3 className="text-lg font-semibold mb-2 text-[#1a365d]">Most Frequent Visitor</h3>
              <p className="text-sm">
                <span className="font-semibold">{report.mostFrequentVisitor.student.name}</span>
                <br />
                Visits: {report.mostFrequentVisitor.visits}
              </p>
            </div>
            <div>
              <h3 className="text-lg font-semibold mb-2 text-[#1a365d]">Total Unique Visitors</h3>
              <p className="text-4xl font-bold text-[#1a365d]">{report.totalUniqueVisitors}</p>
            </div>
          </div>
        </div>
      )}
