...
import { FaUsers, FaClock, FaBook, FaCalendarAlt, FaDownload } from 'react-icons/fa';
...
function Dashboard() {
  ...
  const generatePDF = () => {
    ...
    // Title
    doc.setFontSize(16);
    doc.text('Library Dashboard Report', 105, 20, { align: 'center' });

    // Stats Overview
    doc.setFontSize(12);
    doc.text('Overview:', 10, 40);
    const stats = [
      { title: t('Current Visitors'), value: '42' },
      { title: t('Avg. Visit Duration'), value: '2.5h' },
      { title: t('Books Borrowed'), value: '156' },
      { title: t('Peak Day'), value: t('Thursday') },
    ];
    stats.forEach((stat, index) => {
      doc.text(`${stat.title}: ${stat.value}`, 10, 50 + index * 10);
    });

    // Weekly Data Table
    doc.text('Weekly Traffic Data:', 10, 100);
    const weeklyTable = weeklyData.map((row) => [row.day, row.visits]);
    doc.autoTable({
      startY: 110,
      head: [['Day', 'Visits']],
      body: weeklyTable,
    ...
    // Footer
    doc.setFontSize(10);
    doc.text('Generated by Library Dashboard', 105, doc.internal.pageSize.height - 10, { align: 'center' });

    // Save the PDF
    doc.save('Library_Dashboard_Report.pdf');
  };
  return (
    <div className="dashboard">
      ...
      <main className="main-content">
        ...
        <div className="charts-section bg-[#234678]  text-center font-bold mt-6">
          <button className="download-button  " onClick={generatePDF}>
            <FaDownload className="download-icon inline-block mr-4 " />
            Download Report
          </button>
          {/* Additional chart content here */}
        </div>
      </main>
